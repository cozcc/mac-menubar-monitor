...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
    ...
    self.statusItem.button.toolTip = @"MiniMonitor";

    self.detailsItem = [[NSMenuItem alloc] initWithTitle:@"Loading..." action:nil keyEquivalent:@""];
    self.detailsItem.enabled = NO;

    NSMenuItem *refreshItem = [[NSMenuItem alloc] initWithTitle:@"Refresh Now" action:@selector(refresh) keyEquivalent:@"r"];
    refreshItem.target = self;

    NSMenuItem *quitItem = [[NSMenuItem alloc] initWithTitle:@"Quit MiniMonitor" action:@selector(quit) keyEquivalent:@"q"];
...
- (NSString *)detailTitleForSnapshot:(SystemSnapshot)snapshot {
    ...
    if (!snapshot.hasMemory || snapshot.memoryTotalBytes == 0) {
        return [NSString stringWithFormat:@"%@ | Memory: unavailable", cpu];
    }

    double memoryPercent = (double)snapshot.memoryUsedBytes / (double)snapshot.memoryTotalBytes * 100.0;
    return [NSString stringWithFormat:@"%@ | Memory: %@ / %@ (%.1f%%)",
        cpu,
